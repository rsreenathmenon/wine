...
$SORTING_FIELD = ($_REQUEST['sf']) ? $_REQUEST['sf'] : "users_created";
$SORT_ORDER = ($_REQUEST['so']) ? $_REQUEST['so'] : "ASC";

$SHOW_FILTER = "1";
$PAGE_COUNT = 1;

function sample_fetch_getdata()
{
	/* //////////////////////////////// */
	...
	try 
	{
		...
		foreach($_REQUEST as $input_name=>$input_value)
		{
			if(trim($input_value)!="")
			{
				...
				//if($input_name != 'users_ref' && strpos($input_name,'users')===0)
				if($input_name=="users_firstname" || $input_name=="users_lastname")
				{
				...
				else if(strpos($input_name,'users_')===0)
				{
		...
		{
					// START - Error Reporting through mail
					$page = "brand_summary_mysql.php";
					$mailReturn = errorReportingMail($page,$_POST['mode'], __LINE__ , $_POST['login_check_users_ref'] ,$e);
					// END - Error Reporting through mail
...
$response_data = json_decode(sample_fetch_getdata());
