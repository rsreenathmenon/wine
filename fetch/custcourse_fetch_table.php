...
$SORTING_FIELD = ($_REQUEST['sf']) ? $_REQUEST['sf'] : "custcourse_created";
$SORT_ORDER = ($_REQUEST['so']) ? $_REQUEST['so'] : "ASC";

$SHOW_FILTER = "1";
$PAGE_COUNT = 1;

function custcourse_fetch_getdata()
{
	/* //////////////////////////////// */
	...
	try 
	{
		...
		foreach($_REQUEST as $input_name=>$input_value)
		{
			if(trim($input_value)!="")
			{
				...
				//if($input_name != 'custcourse_ref' && strpos($input_name,'custcourse')===0)
				if($input_name=="custcourse_name")
				{
				...
				else if(strpos($input_name,'custcourse_')===0)
				{
		...
		$pdo_fieldname[] = "custcourse_customer_ref = :custcourse_customer_ref";
		$pdo_fieldvalue[":custcourse_customer_ref"] = mysql_real_escape_string_function(strip_tags(trim($CUS), '<p><font><span><div><ul><li><br><table><thead><tbody><th><tr><td>'));

		$query_input = implode(" AND ",$pdo_fieldname);

		$SHOW_FILTER = "1";
		if($query_input=="custcourse_customer_ref = :custcourse_customer_ref")
		{
			$query_input = "custcourse_customer_ref = :custcourse_customer_ref";
			$SHOW_FILTER = "0";
		}
		...
		{
			$query = 	"SELECT 
							COUNT(DISTINCT custcourse_id) as total
					 	FROM 
							custcourse
					 	WHERE 
					 		".$query_input;
		...
		if($sth->execute($pdo_fieldvalue))
		{
			...
			{
				$row['custcourse_note'] = str_replace(array('\r', '\n'), array(chr(13), chr(10)), $row['custcourse_note']);
...
$response_data = json_decode(custcourse_fetch_getdata());
