	...
	try 
	{
		...
		foreach($_REQUEST as $input_name=>$input_value)
		{
			if(trim($input_value)!="")
			{
				...
				else if(strpos($input_name,'winearoma_')===0)
				{
		...
		{
			$query = 	"SELECT 
							COUNT(DISTINCT winearoma_id) as total
					 	FROM 
							winearoma
					 	WHERE 
					 		".$query_input;
		...
		$query = 	"SELECT 
				 	...
				 	FROM 
						winearoma
						LEFT JOIN winestyle ON (winestyle_ref = winearoma_winestyle_ref)
				 	WHERE 
				 		".$query_input." 
					ORDER BY 
