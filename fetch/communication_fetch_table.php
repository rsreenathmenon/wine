...
$SORTING_FIELD = ($_REQUEST['sf']) ? $_REQUEST['sf'] : "communication_created";
$SORT_ORDER = ($_REQUEST['so']) ? $_REQUEST['so'] : "ASC";

$SHOW_FILTER = "1";
$PAGE_COUNT = 1;

function communication_fetch_getdata()
{
	/* //////////////////////////////// */
	...
	try 
	{
		...
		foreach($_REQUEST as $input_name=>$input_value)
		{
			if(trim($input_value)!="")
			{
				...
				if($input_name=="communication_name")
				{
				...
				else if(strpos($input_name,'communication_')===0)
				{
		...
		{
			$query = 	"SELECT 
							COUNT(DISTINCT communication_id) as total
					 	FROM 
							communication
					 	WHERE 
					 		".$query_input;
		...
		if($sth->execute($pdo_fieldvalue))
		{
			...
			{
				$row['communication_note'] = str_replace(array('\r', '\n'), array(chr(13), chr(10)), $row['communication_note']);
...
$response_data = json_decode(communication_fetch_getdata());
