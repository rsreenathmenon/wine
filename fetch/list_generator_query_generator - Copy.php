...
function list_generator_fetch_getdata()
{
	...
	try 
	{
		...
		foreach($_POST as $input_name=>$input_value)
		{
			//if(trim($input_value)!="")
			{
				if($input_name=="customerTest" && trim($input_value)!="")
				{
					...
					$pdo_fieldname[] = " ".$fieldName." LIKE :".$input_name."";
				...
				if($input_name=="customers_age_start" && trim($input_value)!="")
				{
					...
					$pdo_fieldname[] = " ".$fieldName." <= :".$input_name."";
					$pdo_fieldvalue[":".$input_name] = $fieldValue;
				}
				...
				elseif($input_name=="customers_educationlevel" && count($input_value) > 0)
				{
					...
					$pdo_fieldname[] = " ".$fieldName." IN ( :".$input_name." )";
					$pdo_fieldvalue[":".$input_name] = $fieldValue;
				}
				...
				elseif($input_name=="custpack_sent_start" && trim($input_value)!="")
				{
					...
					$pdo_fieldname[] = " ".$fieldName." IN (SELECT `custpack_customer_ref` FROM `custpack` WHERE `custpack_status_id` = 1 GROUP BY `custpack_customer_ref` HAVING count(1) >=  :".$input_name.") ";
					$pdo_fieldvalue[":".$input_name] = $fieldValue;
				}
				...
				elseif($input_name=="custpack_response_received_start" && trim($input_value)!="")
				{
					...
					$pdo_fieldname[] = " ".$fieldName." IN (SELECT `custpack_customer_ref` FROM `custpack` WHERE `custpack_status_id` = 1 AND custpack_feedback = 1 GROUP BY `custpack_customer_ref` HAVING count(1) >=  :".$input_name.") ";
					$pdo_fieldvalue[":".$input_name] = $fieldValue;
				}
				...
				elseif($input_name=="packresponse_received_start" && trim($input_value)!="")
				{
					...
					$pdo_fieldname[] = " ".$fieldName." IN (SELECT `custpack_customer_ref` FROM `custpack` WHERE `custpack_status_id` = 1 AND custpack_feedback = 1 AND custpack_created <=  :".$input_name."  GROUP BY `custpack_customer_ref` HAVING count(1) >= 0 )";
					$pdo_fieldvalue[":".$input_name] = $fieldValue;
				}
				...
				elseif($input_name=="custpack_avg_days_response_start" && trim($input_value)!="")
				{
					...
					$pdo_fieldname[] = " ".$fieldName." IN ( SELECT `custpack_customer_ref` FROM (SELECT `custpack_customer_ref`,count(1), ROUND(AVG(DATEDIFF(`custpack_feedback_on`, `custpack_created`)),0) as fetchedValue, ROUND(:".$input_name.",0) as roundVal FROM `custpack` WHERE `custpack_status_id` = 1 AND custpack_feedback = 1 GROUP BY custpack_customer_ref HAVING COUNT(1) >= 0 ) as temp_".$input_name." WHERE fetchedValue >= roundVal ) ";
				...
				elseif($input_name=="packresponse_avg_date_start" && trim($input_value)!="")
				{
					...
					$pdo_fieldname[] = " ".$fieldName." IN (SELECT `custpack_customer_ref` FROM `custpack` WHERE `custpack_status_id` = 1 AND custpack_feedback = 1 AND custpack_feedback_on <=  :".$input_name."  GROUP BY `custpack_customer_ref` HAVING count(1) >= 0 )";
					$pdo_fieldvalue[":".$input_name] = $fieldValue;
				}
		...
		$RETURN['total_customers_found'] = $list_generator_total;
