	...
	try 
	{
		...
		foreach($_REQUEST as $input_name=>$input_value)
		{
			if(trim($input_value)!="")
			{
				...
				if($input_name=="custpackmystery_name")
				{
				...
				else if(strpos($input_name,'custpackmystery_')===0)
				{
		...
		$pdo_fieldname[] = "custpackmystery_customer_ref = :custpackmystery_customer_ref";
		$pdo_fieldvalue[":custpackmystery_customer_ref"] = mysql_real_escape_string_function(strip_tags(trim($CUS), '<p><font><span><div><ul><li><br><table><thead><tbody><th><tr><td>'));

		$query_input = implode(" AND ",$pdo_fieldname);

		$SHOW_FILTER = "1";
		if($query_input=="custpackmystery_customer_ref = :custpackmystery_customer_ref")
		{
			$query_input = "custpackmystery_customer_ref = :custpackmystery_customer_ref";
			$SHOW_FILTER = "0";
		}
		...
		{
			$query = 	"SELECT 
							COUNT(DISTINCT custpackmystery_id) as total
					 	FROM 
							custpackmystery
					 	WHERE 
					 		".$query_input;
