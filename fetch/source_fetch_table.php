...
$SORTING_FIELD = ($_REQUEST['sf']) ? $_REQUEST['sf'] : "source_created";
$SORT_ORDER = ($_REQUEST['so']) ? $_REQUEST['so'] : "ASC";

$SHOW_FILTER = "1";
$PAGE_COUNT = 1;

function source_fetch_getdata()
{
	/* //////////////////////////////// */
	...
	try 
	{
		...
		foreach($_REQUEST as $input_name=>$input_value)
		{
			if(trim($input_value)!="")
			{
				...
				else if(strpos($input_name,'source_')===0)
				{
		...
		if($sth->execute($pdo_fieldvalue))
		{
			...
			{
				$row['source_note'] = str_replace(array('\r', '\n'), array(chr(13), chr(10)), $row['source_note']);
...
$response_data = json_decode(source_fetch_getdata());
