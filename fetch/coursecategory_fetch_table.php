...
$SORTING_FIELD = ($_REQUEST['sf']) ? $_REQUEST['sf'] : "coursecategory_created";
$SORT_ORDER = ($_REQUEST['so']) ? $_REQUEST['so'] : "ASC";

$SHOW_FILTER = "1";
$PAGE_COUNT = 1;

function coursecategory_fetch_getdata()
{
	/* //////////////////////////////// */
	...
	try 
	{
		...
		foreach($_REQUEST as $input_name=>$input_value)
		{
			if(trim($input_value)!="")
			{
				...
				if($input_name=="coursecategory_name")
				{
				...
				else if(strpos($input_name,'coursecategory_')===0)
				{
		...
		{
			$query = 	"SELECT 
							COUNT(DISTINCT coursecategory_id) as total
					 	FROM 
							coursecategory
					 	WHERE 
					 		".$query_input;
		...
		if($sth->execute($pdo_fieldvalue))
		{
			...
			{
				$row['coursecategory_note'] = str_replace(array('\r', '\n'), array(chr(13), chr(10)), $row['coursecategory_note']);
...
$response_data = json_decode(coursecategory_fetch_getdata());
