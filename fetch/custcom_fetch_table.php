	...
	try 
	{
		...
		foreach($_REQUEST as $input_name=>$input_value)
		{
			if(trim($input_value)!="")
			{
				...
				else if(strpos($input_name,'custcom_')===0)
				{
		...
		$pdo_fieldname[] = "custcom_customer_ref = :custcom_customer_ref";
		$pdo_fieldvalue[":custcom_customer_ref"] = mysql_real_escape_string_function(strip_tags(trim($CUS), '<p><font><span><div><ul><li><br><table><thead><tbody><th><tr><td>'));

		$query_input = implode(" AND ",$pdo_fieldname);

		$SHOW_FILTER = "1";
		if($query_input=="custcom_customer_ref = :custcom_customer_ref")
		{
			$query_input = "custcom_customer_ref = :custcom_customer_ref";
			$SHOW_FILTER = "0";
		}
		...
		{
			$query = 	"SELECT 
							COUNT(DISTINCT custcom_id) as total
					 	FROM 
							custcom
					 	WHERE 
					 		".$query_input;
		...
		$query = 	"SELECT 
				 	...
				 	FROM 
						custcom
						LEFT JOIN communication ON custcom.custcom_communication_ref = communication.communication_ref
				 	WHERE 
				 		".$query_input." 
					ORDER BY 
