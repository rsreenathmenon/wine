...
$SORTING_FIELD = ($_REQUEST['sf']) ? $_REQUEST['sf'] : "region_created";
$SORT_ORDER = ($_REQUEST['so']) ? $_REQUEST['so'] : "ASC";

$SHOW_FILTER = "1";
$PAGE_COUNT = 1;

function region_fetch_getdata()
{
	/* //////////////////////////////// */
	...
	try 
	{
		...
		foreach($_REQUEST as $input_name=>$input_value)
		{
			if(trim($input_value)!="")
			{
				...
				else if(strpos($input_name,'region_')===0)
				{
		...
		{
			$query = 	"SELECT 
							COUNT(DISTINCT region_id) as total
					 	FROM 
							region
					 	WHERE 
					 		".$query_input;
		...
		$query = 	"SELECT 
						region.*,country_name,states_name
				 	FROM 
						region
						LEFT JOIN country on country_ref = region_country_ref
		...
		if($sth->execute($pdo_fieldvalue))
		{
			while($row = $sth->fetch(PDO::FETCH_ASSOC))
			{
				$row['region_note'] = str_replace(array('\r', '\n'), array(chr(13), chr(10)), $row['region_note']);
				foreach ($row as $key => $value) 
				{
					$row[$key] =  stripslashes($value);
				}
...
$response_data = json_decode(region_fetch_getdata());
