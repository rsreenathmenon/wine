...
function list_generator_fetch_getdata()
{
	/* //////////////////////////////// */
	...
	$list_generator_ref =  "";
	$list_generator_total =  "0";
	$list_generator_customers_id =  "";
	$list_generator_jsondata =  "";


	if(isset($_POST['show_level']) && isset($_POST['list_generator_name']) && trim($_POST['list_generator_name'])!="" && ($_POST['show_level']=='2' || $_POST['show_level']=='5' || $_POST['show_level']=='6'))
	{
		...
		try
		{
			...
			$query = 	"SELECT 
							list_generator_ref,
							list_generator_total,
							list_generator_jsondata,
							list_generator_customers_id
					 	FROM 
							list_generator
					 	WHERE 
					 		list_generator_pack_ref = :list_generator_pack_ref  
						ORDER BY 
							list_generator_id DESC
						LIMIT 
							0 , 1";
			...
			if($sth->execute(array(':list_generator_pack_ref' => trim($_POST['list_generator_name']))))
			{
				while($row = $sth->fetch(PDO::FETCH_ASSOC))
				{

					$list_generator_ref =  $row['list_generator_ref'];
					$list_generator_total =  $row['list_generator_total'];
					$list_generator_customers_id =  $row['list_generator_customers_id'];
					$list_generator_jsondata =  $row['list_generator_jsondata'];

					$fetchedValue = json_decode($row['list_generator_jsondata']);
	...
	$RETURN['list_generator_ref'] = $list_generator_ref;
	...
	// if(isset($_POST['show_level']) && isset($_POST['list_generator_name']) && trim($_POST['list_generator_name'])!="" && ($_POST['show_level']=='2' || $_POST['show_level']=='5' || $_POST['show_level']=='3'))
	// {
	// 	echo "<PRE>";
	// 	print_r($_POST);
	...
	try 
	{
		...
		foreach($_POST as $input_name=>$input_value)
		{
			//if(trim($input_value)!="")
			{
				...
				elseif($input_name=="custpack_sent_start" && trim($input_value)!="")
				{
					...
					$pdo_fieldname[] = " ".$fieldName." IN (SELECT `custpack_customer_ref` FROM `custpack` WHERE `custpack_status_id` = 1 AND custpack_pack_ref LIKE '".$_POST['list_generator_name']."' GROUP BY `custpack_customer_ref` HAVING count(1) >=  ".$fieldValue.") ";
					// $pdo_fieldvalue[":".$input_name] = $fieldValue;
				}
				...
				elseif($input_name=="custpack_response_received_start" && trim($input_value)!="")
				{
					...
					$pdo_fieldname[] = " ".$fieldName." IN (SELECT `custpack_customer_ref` FROM `custpack` WHERE `custpack_status_id` = 1 AND custpack_pack_ref LIKE '".$_POST['list_generator_name']."' AND custpack_feedback = 1 GROUP BY `custpack_customer_ref` HAVING count(1) >=  ".$fieldValue.") ";
					// $pdo_fieldvalue[":".$input_name] = $fieldValue;
				}
				...
				elseif($input_name=="packresponse_received_start" && trim($input_value)!="")
				{
					...
					$pdo_fieldname[] = " ".$fieldName." IN (SELECT `custpack_customer_ref` FROM `custpack` WHERE `custpack_status_id` = 1 AND custpack_pack_ref LIKE '".$_POST['list_generator_name']."' AND custpack_feedback = 1 AND custpack_created <=  ".$fieldValue."  GROUP BY `custpack_customer_ref` HAVING count(1) >= 0 )";
					// $pdo_fieldvalue[":".$input_name] = $fieldValue;
				}
				...
				elseif($input_name=="packresponse_avg_date_start" && trim($input_value)!="")
				{
					...
					$pdo_fieldname[] = " ".$fieldName." IN (SELECT `custpack_customer_ref` FROM `custpack` WHERE `custpack_status_id` = 1 AND custpack_pack_ref LIKE '".$_POST['list_generator_name']."' AND custpack_feedback = 1 AND custpack_feedback_on <=  ".$fieldValue."  GROUP BY `custpack_customer_ref` HAVING count(1) >= 0 )";
					// $pdo_fieldvalue[":".$input_name] = $fieldValue;
				}
				...
				{
					$fieldName = "customers_ref";
					$fieldValue = $_POST['list_generator_name'];
					$fieldValue = '"'.$fieldValue.'"';
		...
		if(isset($_POST['show_level']) && isset($_POST['list_generator_name']) && $_POST['show_level']=='4' && trim($_POST['list_generator_name'])!="")
		{
			...
			try
			{
				...
				$list_generator_total = $temp_data['total'];
				$list_generator_customers_id = $temp_data['totalCustomers'];
			}
			catch(PDOException $e)
			{
			...
			$list_generator_ref = random_reference();
			try
			{
				$query = 	"INSERT INTO
							list_generator
							...
							list_generator_status_id 			= :list_generator_status_id,
							list_generator_name 				= :list_generator_name,
							list_generator_pack_ref 			= :list_generator_pack_ref,
							list_generator_total 				= :list_generator_total,
							list_generator_jsondata 			= :list_generator_jsondata,
							list_generator_customers_id 		= :list_generator_customers_id";
				
				$sth = $PDO_WRITE->prepare($query, array(PDO::ATTR_CURSOR => PDO::CURSOR_FWDONLY));

				$sth->execute(
					array(
						':list_generator_created' =>date('Y-m-d H:i:s'),
						':list_generator_modified' => date('Y-m-d H:i:s'),
						':list_generator_ref' => $list_generator_ref,
						':list_generator_status_id' => '1',
						':list_generator_name' => $_POST['list_generator_name'],
						':list_generator_pack_ref' => $_POST['list_generator_name'],
						':list_generator_total' => $list_generator_total,
						':list_generator_jsondata' => $requestThroughPostToSave,
						':list_generator_customers_id' => $list_generator_customers_id
					));
			}
			catch(PDOException $e)
		...
		if(isset($_POST['show_level']) && isset($_POST['list_generator_name']) && $_POST['show_level']=='5' && trim($_POST['list_generator_name'])!="" && trim($_POST['customers_add_to_custpack'])=="1" && trim($list_generator_customers_id)!="")
		{
			...
			$packCode = getPacksCodeOnly($_POST['list_generator_name']);
			...
			try
			{
				$query = 	"INSERT INTO
							...
							FROM 
							   customers
							WHERE
							   customers_id IN (".$list_generator_customers_id.");
							  ";
				...
				$sth->execute(
					array(
						...
						':custpack_pack_ref' => $_POST['list_generator_name']
					));
			}
			catch(PDOException $e)
			{
		...
		if(isset($_POST['show_level']) && isset($_POST['list_generator_name']) && $_POST['show_level']=='3' && trim($_POST['list_generator_name'])!="")
		{
			////////////////
			// Get Total
			////////////////
...
$response_data = json_decode(list_generator_fetch_getdata());
