...
$SORTING_FIELD = ($_REQUEST['sf']) ? $_REQUEST['sf'] : "winestyle_created";
$SORT_ORDER = ($_REQUEST['so']) ? $_REQUEST['so'] : "ASC";

$SHOW_FILTER = "1";
$PAGE_COUNT = 1;

function winestyle_fetch_getdata()
{
	/* //////////////////////////////// */
	...
	try 
	{
		...
		foreach($_REQUEST as $input_name=>$input_value)
		{
			if(trim($input_value)!="")
			{
				...
				else if(strpos($input_name,'winestyle_')===0)
				{
		...
		{
			$query = 	"SELECT 
							COUNT(DISTINCT winestyle_id) as total
					 	FROM 
							winestyle
					 	WHERE 
					 		".$query_input;
		...
		if($sth->execute($pdo_fieldvalue))
		{
			...
			{
				$row['winestyle_note'] = str_replace(array('\r', '\n'), array(chr(13), chr(10)), $row['winestyle_note']);
...
$response_data = json_decode(winestyle_fetch_getdata());
