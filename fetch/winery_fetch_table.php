...
$SORTING_FIELD = ($_REQUEST['sf']) ? $_REQUEST['sf'] : "winery_created";
$SORT_ORDER = ($_REQUEST['so']) ? $_REQUEST['so'] : "ASC";

$SHOW_FILTER = "1";
$PAGE_COUNT = 1;

function winery_fetch_getdata()
{
	/* //////////////////////////////// */
	...
	try 
	{
		...
		foreach($_REQUEST as $input_name=>$input_value)
		{
			if(trim($input_value)!="")
			{
				...
				else if(strpos($input_name,'winery_')===0)
				{
		...
		{
			$query = 	"SELECT 
							COUNT(DISTINCT winery_id) as total
					 	FROM 
							winery
					 	WHERE 
					 		".$query_input;
		...
		if($sth->execute($pdo_fieldvalue))
		{
			...
			{
				$row['winery_note'] = str_replace(array('\r', '\n'), array(chr(13), chr(10)), $row['winery_note']);
...
$response_data = json_decode(winery_fetch_getdata());
