...
$SORTING_FIELD = ($_REQUEST['sf']) ? $_REQUEST['sf'] : "pack_created";
$SORT_ORDER = ($_REQUEST['so']) ? $_REQUEST['so'] : "ASC";

$SHOW_FILTER = "1";
$PAGE_COUNT = 1;

function pack_fetch_getdata()
{
	/* //////////////////////////////// */
	...
	try 
	{
		...
		$sth = $PDO->prepare($query, array(PDO::ATTR_CURSOR => PDO::CURSOR_FWDONLY));
		$temp_array = array();
		$pdo_fieldvalue = array(':winepackmap_pack_ref' => $REF);

		if($sth->execute($pdo_fieldvalue))
		{
			while($row = $sth->fetch(PDO::FETCH_ASSOC))
			{
				$temp_array[$row['winepackmap_slno']] = $row['winepackmap_wine_ref'];
			}
			$RETURN['winepackmap'] = $temp_array;
	...
	try 
	{
		...
		$pdo_fieldvalue = array(':pack_ref' => $REF);

		if($sth->execute($pdo_fieldvalue))
		{
			while($row = $sth->fetch(PDO::FETCH_ASSOC))
			{
				$row['pack_note'] = str_replace(array('\r', '\n'), array(chr(13), chr(10)), $row['pack_note']);
				foreach ($row as $key => $value) 
				{
					$row[$key] =  stripslashes($value);
				}
...
$response_data = json_decode(pack_fetch_getdata());
