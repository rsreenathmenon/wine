...
$SORTING_FIELD = ($_REQUEST['sf']) ? $_REQUEST['sf'] : "varietal_created";
$SORT_ORDER = ($_REQUEST['so']) ? $_REQUEST['so'] : "ASC";

$SHOW_FILTER = "1";
$PAGE_COUNT = 1;

function varietal_fetch_getdata()
{
	/* //////////////////////////////// */
	...
	try 
	{
		...
		foreach($_REQUEST as $input_name=>$input_value)
		{
			if(trim($input_value)!="")
			{
				...
				else if(strpos($input_name,'varietal_')===0)
				{
		...
		{
			$query = 	"SELECT 
							COUNT(DISTINCT varietal_id) as total
					 	FROM 
							varietal
					 	WHERE 
					 		".$query_input;
		...
		if($sth->execute($pdo_fieldvalue))
		{
			...
			{
				$row['varietal_note'] = str_replace(array('\r', '\n'), array(chr(13), chr(10)), $row['varietal_note']);
...
$response_data = json_decode(varietal_fetch_getdata());
