...
{
	{
		...
		try
		{
			...
			if($sth->execute(array(':list_generator_ref' => trim($_REQUEST['list_generator_ref']))))
			{
		...
		try
		{
			...
			$query = 	"SELECT 
							CONCAT(`customers_firstname`, ' ', `customers_lastname`) as 'Customer Name'
							...
							, customers_ethnicity AS 'Ethnicity'
							, customers_wine_varietal AS 'Wine Varietal'
							, CASE
