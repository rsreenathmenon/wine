...

{
	...
	if ($_POST['winestyle_ref'] == '')
	{
		$_POST['winestyle_ref'] = random_reference();
		$_POST['winestyle_created'] = date('Y-m-d H:i:s');
		$winestyle_ref = $_POST['winestyle_ref'];
		try
		{
			$query = 	"INSERT INTO
						winestyle
						SET
						winestyle_created 	= :winestyle_created,
						winestyle_ref 		= :winestyle_ref";
			
			$sth = $PDO_WRITE->prepare($query, array(PDO::ATTR_CURSOR => PDO::CURSOR_FWDONLY));

			$sth->execute(array(':winestyle_ref' => $winestyle_ref,':winestyle_created' =>date('Y-m-d H:i:s')));
		}
	...
	{
		if($input_name != 'winestyle_ref' && strpos($input_name,'winestyle')===0)
		{
	...
	
	{
		...
		$pdo_addition[':winestyle_modified'] = date('Y-m-d H:i:s');
		$pdo_addition[':winestyle_ref'] = $_POST['winestyle_ref'];
		...
		try
		{
			$query = 	"UPDATE 
						winestyle 
						SET
						winestyle_modified	= :winestyle_modified,
						".$query_input."
						WHERE
						winestyle_ref			= :winestyle_ref";			
			$sth = $PDO_WRITE->prepare($query, array(PDO::ATTR_CURSOR => PDO::CURSOR_FWDONLY));
...
$newURL = HTTP_SERVER."winestyle.php";
